...
__device__ static void _salsa20_8(uint32_t b[16])
{
...
__device__ static void _blockmix_salsa8(uint64_t *dest, const uint64_t *src, uint64_t *b, unsigned r)
{
    cudaMemcpy(b, &src[(2 * r - 1)*8], 64);

#program
    for (unsigned i = 0; i < 2 * r; i += 2) {
        for (unsigned j = 0; j < 8; j++) b[j] ^= src[i * 8 + j];
        _salsa20_8((uint32_t *) b);
        cudaMemcpy(&dest[i * 4], b, 64);
        for (unsigned j = 0; j < 8; j++) b[j] ^= src[i * 8 + 8 + j];
        _salsa20_8((uint32_t *) b);
        cudaMemcpy(&dest[i * 4 + r * 8], b, 64);
    }
}
